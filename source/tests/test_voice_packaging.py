...
class PackagingExcludeTests(unittest.TestCase):
    ...
    def test_release_metadata_is_synchronized(self):
        ...
        self.assertEqual(source_version.group(1), "1.1.0")
        self.assertEqual(installer_version.group(1), "1.1.0")
        self.assertEqual(workflow_version.group(1), "1.1.0")
        ...
        self.assertIn('SNIPVOICE_RELEASE_LABEL: "1.1.0"', texts["workflow"])
        ...
        self.assertIn('installer/Output/SnipvoiceSetup-${{ env.SNIPVOICE_RELEASE_LABEL }}.exe',
                      texts["workflow"])
        self.assertIn("env.SNIPVOICE_CHANNEL == 'stable'", texts["workflow"])
