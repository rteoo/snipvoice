...
def validate_trigger(trigger, existing_triggers, dynamic_trigger_names):
    ...
    if any(ch.isspace() for ch in trigger):
        warnings.append(tr("O trigger contém espaços em branco."))
    elif any(ch in TERMINATOR_CHARS for ch in trigger):
        warnings.append(tr("O trigger contém pontuação/terminador e pode disparar no meio de palavras."))

    if len(trigger) <= 2:
        warnings.append(tr("Trigger muito curto (1–2 caracteres) pode disparar por engano ao digitar."))

    if trigger in dynamic_trigger_names:
        warnings.append(
            tr("Já existe um snippet dinâmico com esse nome; ele tem prioridade e este nunca seria acionado.")
        )
    ...
    for other in existing_triggers:
        ...
        if trigger.endswith(other):
            warnings.append(tr("O trigger termina com um trigger existente ('{other}').", other=other))
        elif other.endswith(trigger):
            warnings.append(tr("Um trigger existente ('{other}') termina com este trigger.", other=other))
